...
Проект анализирует транзакции из Excel-файла, генерирует JSON для веб-страниц и предоставляет сервисы для поиска и подсчётов.

### Модули и функции

#### `utils.py` — вспомогательные функции
- `read_excel_operations(file_path)` — чтение Excel-файла
- `filter_by_date_range(transactions, date_str, period)` — фильтрация по дате (месяц/неделя/год)
- `get_greeting()` — приветствие по времени суток

#### `views.py` — веб-страницы (JSON для фронтенда)
- `main_page(date_time_str)` — главная страница: приветствие, карты (траты, кешбэк), топ-5 транзакций, курсы валют, цены акций
- `events_page(date_time_str, period)` — страница событий: расходы и поступления по категориям, топ-7 категорий, переводы/наличные

#### `services.py` — сервисы
- `search_transactions(transactions, query)` — поиск транзакций по тексту (описание/категория)
- `search_phone_numbers(transactions)` — поиск транзакций с телефонными номерами
- `investment_bank(transactions, limit)` — расчёт суммы для инвесткопилки (округление трат)

### Запуск

Все модули можно запускать напрямую:

```bash
python src/views.py          # демонстрация главной страницы
python src/services.py       # демонстрация сервисов
```

### Настройки для курсовой
Файл user_settings.json в корне проекта определяет валюты и акции для отображения:
