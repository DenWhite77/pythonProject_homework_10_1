...
## Поиск и подсчёт транзакций

### `search_transactions(transactions, search_query)`
Ищет транзакции, в описании которых содержится заданная строка (регистронезависимо). Использует библиотеку `re`.

### `count_transactions_by_category(transactions, categories)`
Подсчитывает количество транзакций в каждой категории с помощью `Counter` из `collections`.

## Интерактивный режим

Запустите `main.py` для работы в режиме вопрос-ответ:
- Выбор источника данных (JSON, CSV, Excel)
- Фильтрация по статусу (регистронезависимо, с повторным вводом при ошибке)
- Сортировка по дате
